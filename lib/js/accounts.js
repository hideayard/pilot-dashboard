...
function saveAutoSyncSettings() {
    ...
    if (totalSeconds < 10 && enabled) {
        Swal.fire({
            icon: "warning",
            title: "Invalid Interval",
            text: "Minimum sync interval is 10 seconds. Setting to 10 seconds.",
            timer: 2000,
            showConfirmButton: false
        });
        totalSeconds = 10;
        document.getElementById('syncSeconds').value = 10;
    ...
    if (enabled) {
        startAutoSync();
        showToast("success", `Auto sync enabled - Interval: ${formatInterval(totalSeconds)}`);
    } else {
        stopAutoSync();
...
function startAutoSync() {
    ...
    if (autoSyncEnabled && autoSyncIntervalMs >= 10000) {
        ...
        console.log(`Auto sync started - Interval: ${autoSyncIntervalMs/1000} seconds`);
    }
}
...
async function performAutoSync() {
            ...
            batch.map(async (acc) => {
                ...
                try {
                    ...
                    if (statusData && statusData.authenticated) {
                        ...
                        if (account) {
                            if (accountInfo) {
                                account.account_balance = accountInfo.balance || account.account_balance || 0;
                                account.account_equity = accountInfo.equity || account.account_equity || 0;
                                account.margin = accountInfo.margin || account.margin || 0;
                                account.free_margin = accountInfo.freeMargin || account.free_margin || 0;
                                account.margin_level = accountInfo.marginLevel || account.margin_level || 0;
                                account.leverage = accountInfo.leverage || account.leverage || "100";
                                account.currency = accountInfo.currency || account.currency || "USD";
                            }
                            
                            const openTrades = statusData.openTrades || [];
                            let totalBuyVolume = 0, totalSellVolume = 0, buyCount = 0, sellCount = 0, floatingProfit = 0;
    ...
    const failCount = results.filter(r => r.status === 'rejected' || (r.value && !r.value.success)).length;
    
    if (successCount > 0) {
        showToast("success", `Auto sync completed: ${successCount} accounts synced${failCount > 0 ? `, ${failCount} failed` : ''}`);
    }
...
function initAutoSync() {
    ...
    if (secondsInput) secondsInput.addEventListener('input', updateIntervalPreview);
    if (minutesInput) minutesInput.addEventListener('input', updateIntervalPreview);
