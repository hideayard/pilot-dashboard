...
async function syncWithMT4(accountId, botName, mt4Login, mt4Password) {
  // Show loading state on the sync button (now in the header)
  ...
  try {
    ...
    const statusData = await getMT4AccountStatus(mt4Login, token);
    ...
    Swal.fire({
      icon: "error",
      title: "Sync Failed",
      html: `<p>Failed to sync account ${accountId}</p><p style="font-size: 12px; color: #6b7280;">${error.message}</p>`,
      confirmButtonColor: "#ef4444",
    });
