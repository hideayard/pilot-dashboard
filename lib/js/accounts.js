...
async function syncWithMT4(accountId, botName, mt4Login, mt4Password) {
  ...
  try {
    ...
    // Fetch account status with the token
    let statusData;
    try {
      statusData = await getMT4AccountStatus(mt4Login, token);
    } catch (statusError) {
      console.error("Failed to fetch status:", statusError);

      // If status fetch fails with 401/403, token might be invalid
      if (
        statusError.message.includes("401") ||
        statusError.message.includes("403") ||
        statusError.message.includes("Unauthorized") ||
        statusError.message.includes("Forbidden")
      ) {
    ...
    if (statusData && statusData.authenticated) {
      // Update account data with MT4 status
      const account = accountsData.find((acc) => acc.account_id == accountId);
      if (account) {
        // Update open trades info
        ...
        account.total_profit = closedProfit;
