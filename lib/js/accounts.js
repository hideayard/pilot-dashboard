    ...
    if (statusData && statusData.authenticated) {
      ...
      if (account) {
        ...
        openTrades.forEach((trade) => {
          ...
          // InstaForex Type Mapping: 1=Buy, 2=Sell, 3=Buy Limit, 4=Sell Limit, 5=Buy Stop, 6=Sell Stop
          // Buy types: 1, 3, 5
          // Sell types: 2, 4, 6
          if (trade.Type === 1 || trade.Type === 3 || trade.Type === 5) {
            buyCount++;
            totalBuyVolume += volume;
          } else if (trade.Type === 2 || trade.Type === 4 || trade.Type === 6) {
            sellCount++;
...
async function viewAccountOrders(accountId, botName) {
  ...
  try {
    ...
    // InstaForex Type Mapping (CORRECTED)
    const typeMap = {
      1: "Buy", // Market Buy Order
      2: "Sell", // Market Sell Order
      3: "Buy Limit", // Pending Buy Limit
      4: "Sell Limit", // Pending Sell Limit
      5: "Buy Stop", // Pending Buy Stop
      6: "Sell Stop", // Pending Sell Stop
      7: "Balance", // Balance operation
      8: "Credit", // Credit operation
      9: "Deposit", // Deposit operation
      10: "Withdrawal", // Withdrawal operation
    };
        ...
        <div style="margin-bottom: 16px;">
          ${orders
            .map((order) => {
              ...
              const isBuy =
                order.type === 1 || order.type === 3 || order.type === 5;
              const isSell =
                order.type === 2 || order.type === 4 || order.type === 6;
